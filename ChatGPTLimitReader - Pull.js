...
window.__lr_booted = true;
  ...
  <div id="lr-header"
    ...
    <div style="font-weight:600;cursor:move;">Limit Render</div>
    <div style="display:flex;gap:6px;">
      <button id="lr-collapse"
  ...
  <div id="lr-body">
    ...
    <div style="display:flex;gap:8px;margin-top:8px;">
      <button id="lr-shrink"
        style="flex:1;padding:6px 8px;border-radius:8px;border:1px solid rgba(0,0,0,0.18);background:#fff;cursor:pointer;">
        Shrink
      </button>
      <button id="lr-restore"
        style="flex:1;padding:6px 8px;border-radius:8px;border:1px solid rgba(0,0,0,0.18);background:#fff;cursor:pointer;">
        Restore
      </button>
    </div>
    ...
    <div style="margin-top:8px;display:flex;align-items:center;gap:6px;">
      <input id="lr-presend" type="checkbox" ${CFG.preSendAutoShrink ? 'checked' : ''}/>
      <label for="lr-presend">Pre-send auto-shrink</label>
    </div>

    <div style="margin-top:6px;display:flex;align-items:center;gap:6px;">
      <input id="lr-freeze" type="checkbox" ${CFG.freezeGuardEnabled ? 'checked' : ''}/>
      <label for="lr-freeze">Freeze guard</label>
    </div>
...
`;
	...
	#lr-panel.lr-hidden #lr-body,
	#lr-panel.lr-hidden #lr-header button {
	  display: none;
	}

	#lr-panel.lr-hidden::after {
	  content: "LR";
	  font-weight: bold;
	}
	
	#lr-panel.lr-expanded #lr-body {
  display: block;
}
	`;
	...
	let panelState = localStorage.getItem('__lr_panel_state') || 'expanded';

	function setPanelState(state) {
	  panel.classList.remove('lr-expanded', 'lr-collapsed', 'lr-hidden');
	  panel.classList.add(`lr-${state}`);
	  panelState = state;
	  localStorage.setItem('__lr_panel_state', state);
	}
	
	setPanelState(panelState);
	
	//隐藏
$('#lr-collapse').addEventListener('click', (e) => {
  if (panelState === 'hidden') return;
  e.stopPropagation();
  setPanelState(panelState === 'collapsed' ? 'expanded' : 'collapsed');
});

$('#lr-hide').addEventListener('click', () => {
...
panel.addEventListener('click', (e) => {
  ...
  const PANEL_POS_KEY = '__lr_panel_pos';
...
function installPanelDrag(panel) {
  const header = panel.querySelector('#lr-header');
  if (!header) return;
  ...
  const onPointerMove = (e) => {
    ...
    const newLeft = clamp(startLeft + dx, 0, Math.max(0, vw - rect.width));
    const newTop  = clamp(startTop + dy, 0, Math.max(0, vh - rect.height));
  ...
  header.addEventListener('pointerdown', (e) => {
    // 点到按钮/输入框就不要触发拖拽（避免误操作）
    const tag = (e.target && e.target.tagName) ? e.target.tagName.toLowerCase() : '';
    if (tag === 'button' || tag === 'input' || tag === 'label') return;

    // 如果你希望 hidden 状态也能拖拽，把这行删掉
    if (panel.classList.contains('lr-hidden')) return;
